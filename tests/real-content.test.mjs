...
test("only the supplied projects remain, with no design samples", async () => {
  for (const [category, count] of [["gallery", 5], ["sound", 4], ["video", 3], ["website", 2]]) {
    ...
    assert.ok(entries.every((p) => p.description === "" && p.year === "" && p.role === ""));
    for (const entry of entries) {
      if (category === "sound") {
        assert.match(entry.audioUrl, /^\/media\/sound\/.*\.mp3$/);
